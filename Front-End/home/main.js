    ...
    fetch(URL).then(re => re.json()).then(res => {
      ...
      for (let i = 0; i < res.data.length; i++) {
        let item = `<div class="item" name="item" id-img="${res.data[i].id}" id_user = "${res.data[i].id_user}"> 
          ...
          <a href="#popup">  <img src="${"https://completeproject.herokuapp.com/" + res.data[i].photo}" class="img" id = "${res.data[i].id}"> </a>
          </div>
      ...
      for (let i = 0; i < res.data.length; i++) {
        items[i].addEventListener("click", e => {
          ...
          fetch(URL +"image/"+ id).then(re => re.json()).then(data => {
            ...
            img.src="https://completeproject.herokuapp.com/"+X.photo
            console.log(X)
